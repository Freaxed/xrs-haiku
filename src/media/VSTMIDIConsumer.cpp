...
#define FRAME 0; //long ((float (uint32_system_time () - fPlugin->GetLastFilterTime ()) * 0.044100f))
...
MIDI_consumer::MIDI_consumer (VSTItem * plugin) : BMidiLocalConsumer (),
	fPlugin (plugin), fEffect (plugin->fEffect), fMidiProducer (0), fFinish (0)
{
	...
	BString	NameBase = "XRS's VST Input ";
	NameBase << " #";
	BString Name;
	int count = 1;
