...
void URLView::Draw( BRect updateRect ) {
	...
	if( Alignment() == B_ALIGN_RIGHT ) {
		left = rect.right - StringWidth( Text() );
		right = rect.right;
	}
...
void URLView::DoBookmarkDrag() {
	...
	dragView->SetHighColor( color.red, color.green, color.blue, 128 );
...
BRect URLView::GetTextRect() {
	...
	float left, right;
	if( Alignment() == B_ALIGN_RIGHT ) {
		left = frame.right - StringWidth( Text() );
		right = frame.right;
	}
	else if( Alignment() == B_ALIGN_CENTER ) {
		left = frame.left + (frame.Width() / 2) - (StringWidth( Text() ) / 2);
		right = left + StringWidth( Text() );
	}
	else {
		left = frame.left;
...
void URLView::LaunchURL() {
	...
	if( IsEmailLink() ) {
		...
		char *box[1];
		box[0]=(char*)malloc(url->Length());
		strcpy(box[0],url->String());
		status_t result = be_roster->Launch( "text/x-email", 1, box );
		//url->UnlockBuffer();
	...
	// Handle an HTTP link.
		...
		box[0]=(char*)malloc(url->Length());
		strcpy(box[0],url->String());
		status_t result = be_roster->Launch( "text/html", 1, box );
		//url->UnlockBuffer();
