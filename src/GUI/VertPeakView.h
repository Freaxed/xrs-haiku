...
VertPeakView will take care of the rest.
min = 0.0
max = 1.0
----------------------------------------------------------*/
#ifndef VERT_PEAK_VIEW_H
#define VERT_PEAK_VIEW_H
...
class VertPeakView : public BView {
public:
								VertPeakView(const char* name,
									bool useGlobalPulse = true,
									bool displayLabels = true);
	virtual						~VertPeakView();
...
private:
			...
			void				_ResizeBackBitmap(int32 height, int32 channels);
			void				_UpdateBackBitmap();
			void				_RenderSpan(uint8* span, uint32 height,
