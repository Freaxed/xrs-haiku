...
#define arts_return_if_fail(A) if(!A) return;

Resampler::Resampler(Refiller *refiller,const void* cook) :
	dropBytes(0), refiller(refiller), pos(0.0), step(1.0), channels(2),
	bits(16),
	block(0), haveBlock(-1),cookie(cook)
{
...
void Resampler::ensureRefill()
{
	...
	if(block == 0)
	{
		missing = bufferSize+sampleSize
				- refiller->read(buffer,bufferSize+sampleSize,cookie);
	...
	else
	{
		 ...
		 */
		if(dropBytes > 0)
			dropBytes -= refiller->read(buffer,dropBytes,cookie);
		...
		{
			missing = bufferSize
					- refiller->read(&buffer[sampleSize], bufferSize,cookie);
...
#define RESAMPLER_STEP()							\
	...
	if(underrun()) return i;  ; 
	
unsigned long Resampler::run(float *left, float *right, unsigned long samples)
{
	ensureRefill();
	unsigned long i = 0;
